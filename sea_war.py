...
size_canvas_x = 500
size_canvas_y = 500
s_x = s_y = 8  # розмір ігрового поля
s_y = 8
...
menu_y = 40
ships = s_x // 2  # максимальна к-сть кораблів
ship_len1 = s_x // 5  # довжина перших типів кораблів
...
# boom - список попадань по короблю противника
...
def on_closing():
    global app_running
    if messagebox.askokcancel("Вихід з гри", "Хочете вийти з гри ?"):
        app_running = False
        tk.destroy()
...
t1 = Label(tk, text="Гравець №2", font=("Helvetica", 16))
...
b0 = Button(tk, text="Показати кораблі гравця  №1", command=button_show_enemy1)
b0.place(x=size_canvas_x + 20, y=30)

b1 = Button(tk, text="Показати кораблі гравця  №2", command=button_show_enemy2)
b1.place(x=size_canvas_x + 20, y=70)

b2 = Button(tk, text="Почати заново!", command=button_begin_again)
...
def add_to_all(event):
    ...
    if ip_x < s_x and ip_y < s_y:
        if points1[ip_y][ip_x] == -1:
            ...
            if check_winner2():
                print("Перемога гравця  №1 (Всі кораблі противника гравця №2 знищені)!!!!!")
...
def generate_ships_list():
    global ships_list
    ships_list = []
    # генеруємо список випадкових кораблів
...
def generate_enemy_ships():
    ...
    # підрахунок спільних довжин кораблів
    ...
    while sum_1_enemy != sum_1_all_ships:
        # обнуляємо масив кораблів ворога
        enemy_ships = [[0 for i in range(s_x + 1)] for i in
                       range(s_y + 1)]  #+1 для доповнюючої лінії справа та знизу для перевірок генерації противника
                ...
                if primerno_x + len <= s_x:
                    for j in range(0, len):
                        try:
                            ...
                            if check_near_ships == 0:  # записуємо в тому випадку якщо немає нічого поруч
